  ...
  return queue.reduce (function(p, io) {
    return p.then (function(accum) {
      ...
      if (io.type === INPUT) {
        ...
        // Register nth expected log line and move on, keeping the thunk as-is
        // Finally, when all log lines have been registered, set appropriate
        // listener on logMediator.emit and run the thunk.
        // Log lines are expected to be called in sequence, with the correct
        // tags and values. Every call to log has <options.logTimeout> time to
        // happen before the assertion fails.
